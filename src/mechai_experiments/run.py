...
def command(study:str,profile:str,workers:int,resume:bool,cfg:dict)->list[str]:
    ...
    elif study=="confidence": cmd += ["--replicates",str(cfg["confidence"]["replicates"]),"--workers",str(workers)]
    elif study=="scalability": cmd += ["--profile",underlying]
    elif study=="reference": cmd += ["--source","submission"]
    return cmd

def main()->None:
    parser=argparse.ArgumentParser(description=__doc__); parser.add_argument("--study",choices=["all",*ORDER],default="core"); parser.add_argument("--profile",choices=["smoke","submission"],default="smoke"); parser.add_argument("--workers",type=int,default=max(1,min(4,(os.cpu_count() or 2)-1))); parser.add_argument("--resume",action="store_true"); args=parser.parse_args()
    if args.workers<1: parser.error("--workers must be positive")
    cfg=load_profile(args.profile); studies=ORDER if args.study=="all" else [args.study]
    if args.profile=="submission": print("Submission profile selected explicitly; existing compatible records can be resumed.",flush=True)
    for study in studies:
        cmd=command(study,args.profile,args.workers,args.resume,cfg); print("Running:"," ".join(cmd),flush=True); subprocess.run(cmd,cwd=ROOT,check=True)
